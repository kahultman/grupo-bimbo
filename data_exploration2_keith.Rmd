...
ggplot(sampled, aes(x=has_return, y= sales_lead_delta)) + geom_boxplot()
ggplot(sampled, aes(x=has_return, y= demand)) + geom_boxplot()
median(sampled$returns_units[sampled$has_return == TRUE])
median(sampled$returns_units[sampled$has_return == FALSE])
...
ggplot(sampled, aes(log(returns_units), demand)) + 
  geom_point() + 
  stat_smooth() +
