...
```{r Responsiveness of Human model, echo=FALSE, message=FALSE, warning=FALSE}
load("sampled_returns.Rdata")

ggplot(sampled, aes(x=hasreturn, y= sales_lead_delta)) + 
  geom_boxplot() + 
  ggtitle("Comparison of product with and without returns on following week's supply")

ggplot(sampled, aes(log(returns_units), sales_lead_delta)) + 
  geom_point() + 
  stat_smooth() +
  ggtitle("Effect of number of returns from previous week on following weeks supply") +
