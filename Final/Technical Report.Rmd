...
Only 3% of weekly product sales involve any returned products. If the human model is responsive to these returns we expect to see a drop in the following week's deliveries. However, it looks like the human model is not responsive to a product with returns or the number of returned units. 
...
We consider this beneficial for our model because we can estimate actual demand rather than predict effects from the current human model. 
...
![Sample Demand Over Time](sample_time_trend.png)
