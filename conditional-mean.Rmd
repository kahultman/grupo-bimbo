...
# Set your train and test data sets
train <- train2
test  <- validate
...
train$log_demand <- log1p(train$demand)

# Find the overall mean demand
overall <- mean(train$log_demand)
...
```{r}
mean_mod <- left_join(train, product_client_depot, by = c("product", "client", "depot"))
...
#mean_mod <- train[product_client_depot]
```
...
submit_cond_mean <- test %>% select(week, depot, channel, route, client, product, demand, pred)
write.csv(submit_cond_mean, file = "submit_cond_mean_validate.csv", row.names = FALSE)
```
